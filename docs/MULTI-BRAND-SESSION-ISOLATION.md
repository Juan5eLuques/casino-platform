...
### **4. SameSite Dinámico según Escenario**

```csharp
// ✅ NUEVO: SameSite dinámico según cross-site o same-site
...
if (isCrossSite)
{
    // Frontend en dominio diferente (e.g., netlify.app → railway.app)
    cookieOptions.SameSite = SameSiteMode.None;  // REQUERIDO para cross-site
}
else
{
    // Same-site o localhost
    cookieOptions.SameSite = SameSiteMode.Lax;   // Mejor seguridad
}
```

**Escenarios:**
- **Cross-Site** (netlify.app → railway.app): `SameSite=None` (permite cookies)
- **Same-Site** (bet30.com → bet30.com): `SameSite=Lax` (mejor seguridad)
- **Localhost**: `SameSite=Lax`

**Beneficios:**
- Compatible con frontends en dominios diferentes
- Máxima seguridad cuando es posible (Lax)
- Funciona en producción con CDNs/Netlify
...
⚠️ **IMPORTANTE**: Localhost (`localhost` o `127.0.0.1`) **NO resolverá ningún brand** automáticamente.
